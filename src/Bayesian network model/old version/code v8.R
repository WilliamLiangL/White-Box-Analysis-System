# Update log
# improve read file function
# file can be selected by using a popup windows
# add a function to output a csv for visualization
# remove outdated code that is no longer used
...
func.selectImportantAtt = function(data.balanced, importance, threshold, target) {
  importance = importance[importance$MeanDecreaseAccuracy >= threshold, ]
  important.nodes = rownames(importance)
  data.important = data.balanced[, c(important.nodes, target)]
  return(data.important)
}

func.output = function(avg.bn, data.balanced, target) {
  arc = arc.strength(avg.bn, data.balanced, criterion = 'bde')
  arc = func.reverse(arc, target)
...
func.reverse = function(arc, from) {
  if (sum(arc$from==from)>0) {
    index = which(arc$from %in% from)
    for (i in index)
      arc = func.reverse(arc, arc[i,]$to)
...
avg.bn = func.structurelearn(data, threshold=0.8, R=5, algorithm="hc",
...
avg.bn = func.structurelearn(data.balanced, threshold=0.8, R=5, algorithm="hc",
                             algorithm.args=list(score = "bde", iss = 10))
...
threshold = median(importance$MeanDecreaseAccuracy)
data.important = func.selectImportantAtt(data.balanced, importance, threshold=threshold, target=target)
avg.bn = func.structurelearn(data.important, threshold=0.8, R=5, algorithm="hc",
                             algorithm.args=list(score = "bde", iss = 10))
