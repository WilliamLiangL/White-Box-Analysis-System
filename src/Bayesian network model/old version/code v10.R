...
func.structurelearn = function(train, threshold, nFold, trainFold, algorithm, algorithm.args) {
  trainSize = floor(trainFold/nFold*nrow(data))
  # apply bootstrap resampling to calculate averaged arc strengh
  boot = boot.strength(data = train, R = nFold, m = trainSize, algorithm = algorithm,
...
avg.bn = func.structurelearn(data, threshold=threshold, nFold, trainFold, algorithm="hc",
...
avg.bn = func.structurelearn(data.balanced, threshold=threshold, nFold, trainFold, algorithm="hc",
                             algorithm.args=list(score = "bde", iss = 1))
